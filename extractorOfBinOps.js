            ...
            enter:function(node, parent) {
                ...
                if (node.type === "BinaryExpression") {
                    ...
                    if (typeof leftName !== "undefined" && typeof rightName !== "undefined") {
                        ...
                        const parsedTree = esprima.parse(buggySurroundingStatementOperand);
                        const leafNodes = [];
                        estraverse.traverse(parsedTree, {
                            enter:function(node, parent) {
                                if (!node.body && !node.arguments){
                        ...
                        //console.log(leafNodes, leafNodes.length);
                        let cpaths = "opnd";
                        for(let i = 0; i < leafNodes.length; i++){
                            for(let j = i + 1; j < leafNodes.length; j++){
                                if (j > i + 3)
                                    continue
                                let temp1 = leafNodes[i].toString().replace(/ /g,'').replace(/,/g,'').replace(/[\r\n]/g,'');
                                let temp2 = leafNodes[j].toString().replace(/ /g,'').replace(/,/g,'').replace(/[\r\n]/g,'');
                                let temp = temp1 + ',' + Math.floor(getRandomArbitrary(-2147483648, 2147483647)).toString() + ',' + temp2;
                                cpaths = cpaths + ' ' + temp;
                            }
                        }

                        if (cpaths.length > 300){
                            return;
                        }

                        binOpsDefs.push(cpaths);
                        //////////////////// END GET BUGGY SMAPLE //////////////////////////////

                        //////////////////// GET NON-BUGGY SAMPLE///////////////////////////////

                        const parsedTreeNonBuggy = esprima.parse(surroundingStatement);
                        const leafNodesNonBuggy = [];
                        estraverse.traverse(parsedTreeNonBuggy, {
                            enter:function(node, parent) {
                                if (!node.body && !node.arguments){
                                    if (node.type === 'Identifier' && node.name)
                                        leafNodesNonBuggy.push(node.name);
                                    if (node.type === 'Literal' && node.value)
                                        leafNodesNonBuggy.push(node.value)
                                }
                            }
                        })
                        if (leafNodesNonBuggy.length === 0)
                            return;

                        //console.log(leafNodes, leafNodes.length);
                        let cpaths = "clean";
                        for(let i = 0; i < leafNodesNonBuggy.length; i++){
                            for(let j = i + 1; j < leafNodesNonBuggy.length; j++){
                                if (j > i + 3)
                                    continue
                                let temp1 = leafNodesNonBuggy[i].toString().replace(/ /g,'').replace(/,/g,'').replace(/[\r\n]/g,'');
                                let temp2 = leafNodesNonBuggy[j].toString().replace(/ /g,'').replace(/,/g,'').replace(/[\r\n]/g,'');
                                let temp = temp1 + ',' + Math.floor(getRandomArbitrary(-2147483648, 2147483647)).toString() + ',' + temp2;
                                cpaths = cpaths + ' ' + temp;
                            }
                        }
